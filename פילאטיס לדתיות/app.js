...
const i18n = {
  he: {
    ...
    coachTitle: "וידאו הדגמה עם מאמנת",
    coachNote: "זה הסרטון האישי שלך עם אפקט אנימה, אווירת סטודיו, מוזיקת רקע וטיימר.",
    coachStartBtn: "התחלת הדגמה",
    coachPauseBtn: "עצירת הדגמה",
    coachNextBtn: "שלב הבא",
    musicOnBtn: "הפעלת מוזיקה",
    musicOffBtn: "כיבוי מוזיקה",
    coachPlaceholder: "שלב הדגמה יופיע כאן",
    coachCurrentStep: (step) => `עכשיו: ${step}`,
    coachTapPlay: "אם הסרטון לא מתחיל, לחצי על כפתור Play בנגן.",
    timerText: (elapsed, total, remain) => `${elapsed} / ${total} • נשאר ${remain}`,
  ...
  en: {
    ...
    coachTitle: "Coach video demo",
    coachNote: "Your personal video with anime effect, studio look, music, and live timer.",
    coachStartBtn: "Start demo",
    coachPauseBtn: "Pause demo",
    coachNextBtn: "Next step",
    musicOnBtn: "Play music",
    musicOffBtn: "Stop music",
    coachPlaceholder: "Demo step will appear here",
    coachCurrentStep: (step) => `Now: ${step}`,
    coachTapPlay: "If video does not start, press Play in the video controls.",
    timerText: (elapsed, total, remain) => `${elapsed} / ${total} • left ${remain}`,
...
const coachVideoLocal = document.getElementById("coachVideoLocal");
...
function renderStaticTexts() {
  ...
  musicToggleBtn.textContent = musicEnabled ? lang.musicOffBtn : lang.musicOnBtn;
...
function updateVideoTimer() {
  const lang = t();
  const elapsed = formatTime(coachVideoLocal.currentTime || 0);
  const total = formatTime(coachVideoLocal.duration || 0);
  const remain = formatTime((coachVideoLocal.duration || 0) - (coachVideoLocal.currentTime || 0));
  videoTimer.textContent = lang.timerText(elapsed, total, remain);
}

function playSoftTone(time, frequency, duration, gainValue) {
...
function startBackgroundMusic() {
  ...
  const beatSeconds = 1.2;
  const playBeat = () => {
    const now = audioContext.currentTime;
    playSoftTone(now, 220, 0.28, 0.022);
    playSoftTone(now + 0.25, 261.63, 0.22, 0.018);
    playSoftTone(now + 0.6, 329.63, 0.24, 0.017);
  };

  playBeat();
  musicBeatId = setInterval(playBeat, beatSeconds * 1000);
}

function stopBackgroundMusic() {
  if (musicBeatId) {
    clearInterval(musicBeatId);
    musicBeatId = null;
  }
}
...
function startCoachDemo() {
  ...
  coachPlaying = true;
  coachStepIndex = -1;
  const playResult = coachVideoLocal.play();
  if (playResult && typeof playResult.catch === "function") {
    playResult.catch(() => {
      coachStepText.textContent = t().coachTapPlay;
    });
  }
  nextCoachStep();
...
function nextCoachStep() {
  ...
  if (Number.isFinite(coachVideoLocal.duration) && coachVideoLocal.duration > 0) {
    const segmentLength = coachVideoLocal.duration / plan.routine.length;
    coachVideoLocal.currentTime = Math.min(
      coachVideoLocal.duration - 0.1,
      coachStepIndex * segmentLength
    );
  }
  renderCoachStep(day);
}
...
coachVideoLocal.addEventListener("loadedmetadata", updateVideoTimer);
coachVideoLocal.addEventListener("timeupdate", updateVideoTimer);
coachVideoLocal.addEventListener("ended", () => {
  coachPlaying = false;
  renderCoachStep(Number(dayPicker.value || getSuggestedDay()));
});
